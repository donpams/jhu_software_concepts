...
class GradCafeScraper:
    ...
    def check_robots(self, screenshot_path: Optional[Path] = Path("screenshot.jpg")) -> bool:
        ...
        if screenshot_path:
            ...
            try:  # convert to .jpg as the assignment asks (Pillow ships with selenium deps)
                ...
                pass
            Path(screenshot_path).with_name("robots.txt").write_text(body, encoding="utf-8")
        return allowed
    ...
    def scrape_new_entries(
        ...
        """Fetch entries that are not yet in ``known_urls`` (Module 3 Pull Data).

        Grad Cafe lists newest submissions first, so we start at page 1 and
        follow "Next" links; as soon as a page contributes no new entry we
        have reached data we already hold and stop.  ``max_pages`` is a
        safety cap.  Returns the new records (also saved to ``output`` when
        given).  The site's polite-delay and stop-on-block behaviour is the
        same as in ``scrape_data``.
        """
        new_records: List[Dict[str, Any]] = []
        url = build_survey_url(1)
        for page in range(1, max_pages + 1):
            try:
                ...
                break
            page_records = self._parse_page(html)
            fresh = [r for r in page_records if r["url"] and r["url"] not in known_urls]
            known_urls.update(r["url"] for r in fresh)
            new_records.extend(fresh)
            print(f"page {page:3d}: {len(fresh):3d} new of {len(page_records)}")
            next_url = self._find_next_url(html)
            if not fresh or not page_records or next_url is None:
                break
            url = next_url
            time.sleep(self.delay_seconds)
        if output is not None:
            save_data(new_records, output)
        return new_records
